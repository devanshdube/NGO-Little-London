...
exports.registerSchool = async (req, res) => {
  ...
  if (!name || !mobile || !email || !password) {
    return res.status(400).json({
      status: "Failure",
      message: "Required fields:  name, mobile, email and password",
    });
  }

  try {
    db.query(
      "SELECT * FROM school_admin WHERE email = ? OR name = ?",
      [email, name],
      async (err, results) => {
        if (err) {
          return res
        ...
        db.query(
          ...
          (name, mobile, email, password, created_at)
          VALUES (?, ?, ?, ?, ?)`,
          [name, mobile, email, hashedPassword, createdAt],
          (err, result) => {
            if (err) {
              return res.status(500).json({
                status: "Failure",
            ...
            res.status(201).json({
              ...
              data: { id: result.insertId, name, mobile, email },
            });
          }
        );
      }
...
exports.loginSchool = async (req, res) => {
  ...
  try {
    const getUserQuery = "SELECT * FROM school_admin WHERE email = ?";
    db.query(getUserQuery, [email], async (err, results) => {
      if (err) {
        console.error("Database error:", err);
        return res.status(500).json({
...
exports.forgotSchoolPassword = async (req, res) => {
  try {
    const { email } = req.body;
    if (!email) {
      return res
    ...
    const getUserQuery = `SELECT * FROM school_admin WHERE email = ? LIMIT 1`;
    db.query(getUserQuery, [emailLower], async (err, result) => {
      const genericResponse = () =>
        res.status(200).json({
          status: "Success",
...
exports.verifyOtpAndResetSchoolPassword = async (req, res) => {
  try {
    const { email, otp, newPassword } = req.body;
    if (!email || !otp || !newPassword) {
      return res.status(400).json({
    ...
    const updatePasswordQuery = `UPDATE school_admin SET password = ? WHERE email = ?`;
    db.query(
      updatePasswordQuery,
      [hashedPassword, emailLower],
      (updateErr, updateResult) => {
